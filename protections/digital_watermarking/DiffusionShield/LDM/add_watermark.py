...
def process_images(block_width, image_size, dir_wmlist, dataset, save_dif, wm_patches_dir):
    vae = AutoencoderKL.from_pretrained("/data/xunaen/Text-to-image-Copyright/stable-diffusion-1-5", subfolder="vae", revision=None)
    vae = vae.to('cuda')
    ...
    #print(wmpatches.shape)
    #print(type(wmpatches))

    if type(dir_wmlist)==str:
        wmlist = torch.load(dir_wmlist)
    else: 
        wmlist= dir_wmlist
    count = int((image_size/8) / block_width)
    ...
    for data, _ in loader: 
        ...
        for i in range(count):
            for j in range(count):
                ...
                if wmlist[index]!=0:
                    ...
                    print(latents[0][:,i*block_width:(i+1)*block_width, j*block_width:(j+1)*block_width].shape)
                    print(wmpatches[0].shape)
                    print(wmpatches[0])'''
        ...
        cv2.imwrite(f'{save_dif}/{m}.png', img * 255)
        m+=1
...
def main():
    ...
    parser.add_argument("--dataset", type=str, default='/data/xunaen/lmx/data/person_for_dreambooth', help="Dataset: if not cifar10, indicate the directory")
    parser.add_argument("--save_dif", type=str, default='./watermarked_img/person/person')
    parser.add_argument("--wmpatches_dir", type=str, default='./wm_patches.pt')
