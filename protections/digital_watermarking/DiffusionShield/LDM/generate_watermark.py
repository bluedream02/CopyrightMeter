...
class generate_watermark(Base):
    def __init__(self, model,
        ...
        self.count=int((image_size/8)/patch_size)
        self.augment_type=augment_type
        self.model_save_path = model_save_path
        self.patches_save_path = patches_save_path
        self.vae = AutoencoderKL.from_pretrained("/data/xunaen/Text-to-image-Copyright/stable-diffusion-1-5", subfolder="vae", revision=None)
    ...
    def generate_wm(self, train_loader, test_loader):
        ...
        #wm_patches=(self.B)*list(initial)
        ...
        for data, _ in test_loader:
            ...
            for m in range(latents.shape[0]):  
                for i in range(0,self.count):#8
                    for j in range(0,self.count):#8
                        if randomlist[self.count*i+j]!=0:
                            #print(latents[m,:,i*self.patch_size:i*self.patch_size+self.patch_size,j*self.patch_size:j*self.patch_size+self.patch_size].shape)
                            #print(wm_patches[randomlist[self.count*i+j]-1].shape)
    ...
    def update_wm(self, wm_patches, data, target):
        ...
        #print(wm_patches[0].shape)
        #print(wm_patches[0])
        #print(len(wm_patches))
        for m in range(len(wm_patches)):
            if wm_patches[m].grad!=None:
