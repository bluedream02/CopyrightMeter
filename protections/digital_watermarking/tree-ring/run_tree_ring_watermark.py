...
def main(args):
    ...
    df = pd.read_csv(f"/path/{data}_prompt.csv")

    for i in tqdm(range(args.start, args.end)):
        seed = i + args.gen_seed
        
        #current_prompt = dataset[i][prompt_key]
        current_prompt = df['Prompt'][i]
        ### generation
        ...
        os.makedirs(f"./{data}/origin_image", exist_ok=True)
        orig_image_no_w.save(f"./tree-ring/{data}/origin_image/{i}.png")
        ...
        os.makedirs(f"./tree-ring/{data}/watermark_image", exist_ok=True)
        orig_image_w.save(f"./tree-ring/{data}/watermark_image/{i}.png")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_id', default='/path/stable-diffusion-1-5')
