...
def process_directory(args):
    ...
    #     print("[is_traverse_subdirectories]")
    ...
    # else:
    #     print("args.image_directory_path:",args.image_directory_path)
...
def process_single_directory(dir_path, args):
    ...
    with open(result_file_path, "a") as result_file:
        ...
        if processed_images > 0:
            ...
            #with open(os.path.join(dir_path, "result.txt"), "a") as parent_result_file:
            print("dir_path: ", dir_path)
            print("os.path.dirname(dir_path): ",os.path.dirname(dir_path))

            with open(os.path.join(dir_path, "result.txt"), "a") as parent_result_file:
                parent_result_file.write(f"{os.path.basename(dir_path)}, Average Bit Accuracy, {average_bit_accuracy}\n")

            p2 = os.path.dirname(dir_path)
            p2 = os.path.dirname(p2)
            p2 = os.path.join(p2, "final_result.txt")
            with open(p2, "a") as result_file:
                #result_file.write(f"{p2}, Average Bit Accuracy, {average_bit_accuracy}\n")
                result_file.write(f"{average_bit_accuracy}\n")
