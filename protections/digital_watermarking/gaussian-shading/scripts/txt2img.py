...
def main(opt):
    ...
    with torch.no_grad(), \
        ...
        model.ema_scope():
            ...
            for n in trange(opt.n_iter, desc="Sampling"):
                ...
                for prompts in tqdm(data, desc="data"):
                    ...
                    for x_sample in x_samples:
                        ...
                        x_sample = x_sample.type(torch.float32)  # 将张量转换为Float32类型
                        x_sample = x_sample.cpu().numpy().astype('float32') * 255.
                        x_sample = rearrange(x_sample, 'c h w -> h w c')

                        #x_sample = 255. * rearrange(x_sample.cpu().float().numpy().astype(ml_dtypes.bfloat16), 'c h w -> h w c')
            ...
            grid = rearrange(grid, 'c h w -> h w c').type(torch.float32)  # 将张量转换为Float32类型
            grid = 255. * grid.cpu().numpy()
