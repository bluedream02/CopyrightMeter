...
# Set up logging
logging.basicConfig(filename='training_log1.log', level=logging.INFO, format='%(asctime)s %(message)s')
...
parser.add_argument(
    "--ori_dir",
    type=str,
    default="./traindata_p0.0_none/train_person/",
)
parser.add_argument(
    "--coated_dir",
    type=str,
    default="./traindata_p1.0_wanet_unconditional_s2.0_k128_removeeval/train_person/",
)
parser.add_argument(
...
logging.info(args.generated_inspected_dir)
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(trainloader):
        ...
        loss.backward()
        optimizer.step()
    logging.info(f"Time for this epoch: {time.time() - strat_time}")
...
def test():
    ...
    for j in range(num_classes):
        logging.info(f"class {j} correct num: {correct_class[j]}")
    ...
    with torch.no_grad():
        ...
        logging.info(f"memorization_strength for the inspected model: {memorization_strength_acc / memorization_strength_list.shape[0]}")

        if (memorization_strength_acc / memorization_strength_list.shape[0]) > 0.75:
            logging.info("malicious model!")
        else:
            logging.info("benign model")
...
def cal_fid():
    ...
    logging.info(f"Fake images shape: {fake_images.shape}")
    fid = FrechetInceptionDistance(normalize=True)
    fid.update(real_images, real=True)
    fid.update(fake_images, real=False)
    logging.info(f"FID: {float(fid.compute())}")
