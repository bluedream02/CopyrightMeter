...
clip_model = CLIPModel.from_pretrained("/data/xunaen/llms/clip-vit-base-patch32")
clip_processor = CLIPProcessor.from_pretrained("/data/xunaen/llms/clip-vit-base-patch32")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', help='datasets', type=str, required=False, default=False)
    ...
    sd14="/data/xunaen/Text-to-image-Copyright/stable-diffusion-1-5"
    sd21='/data/xunaen/llms/stable-diffusion-2-1'
    if args.base=='1.4':
        model_version = sd14
    elif args.base=='2.1':
        model_version = sd21
    ...
    #torch.save(ldm_stable.unet.state_dict(), f'models/{args.dataset}/erased-{print_text}.pt')
    torch.save(ldm_stable.unet.state_dict(), f'models/{args.dataset}/{concepts[0]}.pt')

    if not os.path.exists(f'info/{args.dataset}'):
        os.makedirs(f'info/{args.dataset}')

    # with open(f'info/{args.dataset}/erased-{print_text}.txt', 'w') as fp:
    #     json.dump(concepts,fp)

    with open(f'info/{args.dataset}/{concepts[0]}.txt', 'w') as fp:
        json.dump(concepts,fp)
