...
def parse_args():
    ...
    parser.add_argument("--model_path", type=str, help="Path to model checkpoint", default="/path/model/stable-diffusion-1-5")
...
if __name__ == "__main__":
    ...
    if(args.mode == "test"):
        ...
        pipe = SLDPipeline.from_pretrained(
            "/path/model/stable-diffusion-1-5", safety_checker=None,
        ).to(device)    

        if args.model_path != "/path/model/stable-diffusion-1-5": 
        ...
        if model_path != "/path/model/stable-diffusion-1-5": 
            text_encoder = CLIPTextModel.from_pretrained(args.model_path, subfolder="text_encoder", revision=False).to("cuda")
            tokenizer = CLIPTokenizer.from_pretrained(args.model_path, subfolder="tokenizer")
            pipe.text_encoder = text_encoder
            pipe.tokenizer = tokenizer
