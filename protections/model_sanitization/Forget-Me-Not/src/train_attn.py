...
class ForgetMeNotDataset(Dataset):
    ...
    def __init__(
        ...
        for c, t, num_tok in multi_concept:
            p = Path("/data/xunaen/Text-to-image-Copyright/dataset/update_lmx/origin_data/artist", c) #原始数据
            if not p.exists():
            ...
            if t == "object":
                ...
                self.instance_prompt += [(f"a painting in the style of {target_snippet}", target_snippet)] * len(image_paths)
            else:
...
def main(args):
    ...
    if  args.use_ti:
        ...
        for c, t in args.multi_concept:
            ...
            weight_path = f"/data/xunaen/Text-to-image-Copyright/dataset/update_lmx/origin_data/exps_ti/artist/{c}/step_inv_500.safetensors"
