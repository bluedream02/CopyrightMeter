...
class RandomCropAttacker(WMAttacker):
    ...
    def attack(self, image_paths, out_paths, multi=False):
        for (img_path, out_path) in tqdm(zip(image_paths, out_paths)):
            ...
            w, h = img.size
            patch_w = int(w * (1 - self.percent))
            patch_h = int(h * (1 - self.percent))
            
            patched_img = img.copy()
            draw = ImageDraw.Draw(patched_img)
            draw.rectangle([0, 0, w, patch_h], fill='black')
            draw.rectangle([w - patch_w, 0, w, h], fill='black')
            patched_img.save(out_path)


class RandomDropAttacker(WMAttacker):
    def __init__(self, percent=0.6):
        self.percent = percent
    ...
    def attack(self, image_paths, out_paths, multi=False):
        for (img_path, out_path) in tqdm(zip(image_paths, out_paths)):
            ...
            img = Image.open(img_path)
            w, h = img.size
            patch_w, patch_h = int(w * self.percent), int(h * self.percent)
            x = np.random.randint(0, w - patch_w)
            y = np.random.randint(0, h - patch_h)
            
            patched_img = img.copy()
            draw = ImageDraw.Draw(patched_img)
            draw.rectangle([x, y, x + patch_w, y + patch_h], fill='black')
            patched_img.save(out_path)
...
class MedianFilterAttacker(WMAttacker):
    ...
    def attack(self, image_paths, out_paths, multi=False):
        for (img_path, out_path) in tqdm(zip(image_paths, out_paths)):
            ...
            img = Image.open(img_path)
            filtered_img = img.filter(ImageFilter.MedianFilter(self.size))
...
class SaltPepperNoiseAttacker(WMAttacker):
    ...
    def attack(self, image_paths, out_paths, multi=False):
        for (img_path, out_path) in tqdm(zip(image_paths, out_paths)):
            ...
            img = Image.open(img_path)
            img_array = np.array(img)
            salt_pepper = np.random.choice([0, 255], img_array.shape[:2], p=[self.prob, 1 - self.prob])
            for i in range(img_array.shape[2]):
                img_array[:, :, i] = img_array[:, :, i] * (salt_pepper != 0) + salt_pepper * (salt_pepper == 0)
            noisy_img = Image.fromarray(img_array)
            noisy_img.save(out_path)


class ResizeRestoreAttacker(WMAttacker):
    def __init__(self, scale_percent=25):
    ...
    def attack(self, image_paths, out_paths, multi=False):
        for (img_path, out_path) in tqdm(zip(image_paths, out_paths)):
            ...
            img = Image.open(img_path)
            w, h = img.size
            resized_img = img.resize((int(w * self.scale_percent / 100), int(h * self.scale_percent / 100)))
            restored_img = resized_img.resize((w, h))
...
class BrightnessAttacker(WMAttacker):
    ...
    def attack(self, image_paths, out_paths, multi=False):
        for (img_path, out_path) in tqdm(zip(image_paths, out_paths)):
            ...
            brightened_img = enhancer.enhance(self.factor)
            brightened_img.save(out_path)
